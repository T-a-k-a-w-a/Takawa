--================================================================--
--      SKRIP VERSI FINAL (WINDUI + SEMUA PERBAIKAN) - OLEH PARTNER CODING     --
--================================================================--
...
-- Variabel Anti Fall Damage
local isFalling = false
local startFallY = 0
local fallDamageThreshold = 40 -- Jarak jatuh minimum untuk memicu heal
...
RunService.Heartbeat:Connect(function()
    ...
    -- Logika Terbang untuk Mobile
    if flying and rootPart then
        local camera = workspace.CurrentCamera; local moveDirection = humanoid.MoveDirection; local flyVelocity = Vector3.new(moveDirection.X, 0, moveDirection.Z).Unit * flySpeed
    ...
    -- !-- PERBAIKAN: Logika baru Anti Fall Damage --!
    if antiFallDamageEnabled then
        if humanoid.FloorMaterial == Enum.Material.Air and rootPart.Velocity.Y < -30 then
            if not isFalling then
                isFalling = true
                startFallY = rootPart.Position.Y
            end
        else
            if isFalling then
                isFalling = false
                local fallDistance = startFallY - rootPart.Position.Y
                if fallDistance > fallDamageThreshold then
                    humanoid.Health = humanoid.MaxHealth -- Heal instan
                end
            end
        end
    end
...
--                         UI TABS & ELEMENTS                       --
--================================================================--
-- Struktur UI Diubah Agar Tidak Macet
...
TabTeleport:Button({ Title = "Pindahkan Teman ke Saya", Callback = function()
    ...
    local friendPlayer = findPlayer(friendName)
...
TabAdvanced:Button({
    Title = "Activate Anti-Cheat Bypass",
