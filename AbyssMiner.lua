--================================================================--
--      SKRIP VERSI FINAL (WINDUI + MOBILE FIX) - OLEH PARTNER CODING     --
--================================================================--
...
local windowHeight = math.min(screenSize.Y * 0.8, 480)
...
local function startFly()
    local char = Player.Character; if not char or not char:FindFirstChild("HumanoidRootPart") or flying then return end;
    ...
    flying = true
    Player.DevEnableMouseLock = true -- Mengaktifkan shift lock otomatis
    originalMouseBehavior = UserInputService.MouseBehavior
    UserInputService.MouseBehavior = Enum.MouseBehavior.LockCenter
end
local function stopFly()
    if bodyGyro then bodyGyro:Destroy() end; if bodyVelocity then bodyVelocity:Destroy() end; flying = false
    Player.DevEnableMouseLock = false
    if originalMouseBehavior then UserInputService.MouseBehavior = originalMouseBehavior end
end

-- Fungsi pencarian player (bisa pakai username atau display name)
local function findPlayer(name)
    local targetName = name:lower()
    for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
        if player.Name:lower():match(targetName) or player.DisplayName:lower():match(targetName) then
            return player
        end
    end
    return nil
...
RunService.Heartbeat:Connect(function()
    ...
    if flying and char:FindFirstChild("HumanoidRootPart") then
        local rootPart = char.HumanoidRootPart; local camera = workspace.CurrentCamera;
        local moveDirection = humanoid.MoveDirection
        local flyVelocity = Vector3.new(moveDirection.X, 0, moveDirection.Z).Unit * flySpeed
        
        if uiElements.FlyUp and uiElements.FlyUp.Button.MouseButton1Down then
            flyVelocity = flyVelocity + Vector3.new(0, flySpeed, 0)
        end
        if uiElements.FlyDown and uiElements.FlyDown.Button.MouseButton1Down then
            flyVelocity = flyVelocity - Vector3.new(0, flySpeed, 0)
        end
...
Player.CharacterAdded:Connect(function(char) 
    local humanoid = char:WaitForChild("Humanoid"); humanoid.StateChanged:Connect(function(old, new) onStateChanged(humanoid, old, new) end)
    task.wait(2)
    pcall(function()
...
uiElements.Fly = TabPlayer:Toggle({ Name = "Toggle Fly", Desc = "Otomatis shift-lock. Gunakan joystick untuk arah.", Default = false, Callback = function(state) flyEnabled = state; if state then startFly() else stopFly() end end })
uiElements.FlyUp = TabPlayer:Button({ Name = "Naik (Tahan)", Desc = "Tahan untuk terbang ke atas." })
uiElements.FlyDown = TabPlayer:Button({ Name = "Turun (Tahan)", Desc = "Tahan untuk terbang ke bawah." })
...
TabTeleport:Button({ Name = "Pindahkan Teman ke Saya", Callback = function()
    ...
    local friendPlayer = findPlayer(friendName) -- Menggunakan fungsi pencarian baru
